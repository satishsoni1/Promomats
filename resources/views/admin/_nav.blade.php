@php($scopeDept = auth()->user()?->adminDepartmentScope())
<div class="flex gap-4 border-b border-gray-200 mb-6 text-sm overflow-x-auto items-center">
    ...
    @if ($scopeDept)
        <span class="ml-auto pb-2 shrink-0 text-xs px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 whitespace-nowrap">Dept admin · {{ $scopeDept }}</span>
    @endif
</div>
